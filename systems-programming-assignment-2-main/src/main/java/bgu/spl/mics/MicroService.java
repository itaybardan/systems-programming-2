...
public abstract class MicroService implements Runnable {
    ...
    protected CopyOnWriteArrayList<Event> tasks; //Services that have their own tasks will add mesasges.
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        messages_callbacks.put(type, callback);
                ...
                if (!tasks.isEmpty()) {
                    for (Event e : tasks) {
                        if (e.getFuture().isDone()) {
                            messages_callbacks.get(e.getClass()).call(e);
                        }
                    }
                    if(messageBus.isMessageQueueEmpty(this)) continue; //The only tasks might be store already on this ms, will not wait for bus to send a message.
                }
